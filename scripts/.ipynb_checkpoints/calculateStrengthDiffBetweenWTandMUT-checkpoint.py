...
"""
with open("../tmp/Model_MAPTsplicing_MAPTExon10Intron10withDMSdata_SREmotifs/MAPT_SingleMutations_TrainingSet_5pSpliceSitesScores.tsv") as f:
    singleMuts=[line.strip().split("\t") for line in f][1:]

with open("../tmp/Model_MAPTsplicing_MAPTExon10Intron10withDMSdata_SREmotifs/MAPT_DoubleMutations_TrainingSet_5pSpliceSitesScores.tsv") as f:
    doubleMuts=[line.strip().split("\t") for line in f][1:]
...
with open("../results/MAPT_ModelFeature_AllMutations_WTvsMUT_DiffIn5pSpliceSiteStrength.tsv","w") as fw:
    fw.write("MutID"+"\t"+"SS"+"\n")
    for i in to_write:
        fw.write("\t".join(i))
        fw.write("\n")
...
# This is for SREs
"""
with open("../tmp/Model_MAPTsplicing_MAPTExon10Intron10withDMSdata_SREmotifs/MAPT_SingleMutations_TrainingSet_SREstrengthsDifferences_perCluster.tsv") as f:
    singleMuts=[line.strip().split("\t") for line in f][1:]

with open("../tmp/Model_MAPTsplicing_MAPTExon10Intron10withDMSdata_SREmotifs/MAPT_DoubleMutations_TrainingSet_SREstrengthsDifferences_perCluster.tsv") as f:
    doubleMuts=[line.strip().split("\t") for line in f][1:]
    
to_write=singleMuts+doubleMuts

dict_NumCluster={"ESE":8,"ESS":7,"ISE":7,"ISS":8}
    
with open("../tmp/Model_MAPTsplicing_MAPTExon10Intron10withDMSdata_SREmotifs/MAPT_ModelFeature_AllMutations_WTvsMUT_DiffStrengthOfSREs_PerCluster.tsv","w") as fw:
    fw.write("MutID")
...
sre_scores = pd.read_csv("../tmp/Model_MAPTsplicing_MAPTExon10Intron10withDMSdata_SREmotifs/MAPT_ModelFeature_AllMutations_WTvsMUT_DiffStrengthOfSREs_PerCluster.tsv",header=0,sep="\t")
...
strengths_Muts_DF.iloc[:,[4,0,1,2,3]].to_csv("../results/MAPT_ModelFeature_AllMutations_WTvsMUT_DiffStrengthOfSREs_MeanOfClustersPerMotifType.tsv",sep="\t",header=True,index=False)
"""
